...
// audio player - CPO page
function toggleAudioCpo() {
    const audio = document.getElementById('audioPlayerCpo');
    const playIcon = document.getElementById('playIconCpo');
    const pauseIcon = document.getElementById('pauseIconCpo');
    if (!audio) return;
    ...
    audio.addEventListener('timeupdate', () => {
        const progress = (audio.currentTime / audio.duration) * 100;
        document.getElementById('audioProgressCpo').style.width = progress + '%';
        const mins = Math.floor(audio.currentTime / 60);
        const secs = Math.floor(audio.currentTime % 60).toString().padStart(2, '0');
        const durMins = Math.floor(audio.duration / 60);
        const durSecs = Math.floor(audio.duration % 60).toString().padStart(2, '0');
        document.getElementById('audioTimeCpo').textContent = mins + ':' + secs + ' / ' + durMins + ':' + durSecs;
    });
    ...
    audio.addEventListener('ended', () => {
        ...
        playIcon.style.display = 'block';
        pauseIcon.style.display = 'none';
        document.getElementById('audioProgressCpo').style.width = '0%';
        document.getElementById('audioTimeCpo').textContent = '0:00';
    });
}
